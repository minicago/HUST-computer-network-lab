...
int handle_request(char* recvbuf, int len, SOCKET socket){
	char *ptr = recvbuf;
	char requestType[4096], fileName[4096], httpVer[4096], sendbuf[4096];
	strcpy(fileName, rootpath);
	for(;sscanf(ptr,"%s %s %s",requestType, fileName + strlen(rootpath), httpVer) != EOF;){
		ptr+=strlen(requestType) + strlen(fileName) + strlen(httpVer) + 2;
		if(strcmp(requestType, "GET") == 0){
			...
			if(file != NULL){
				...
				printf("%s",sendbuf);
				msend(sendbuf , socket);
				int total;
				while (true){
					int rtn = fread((void*) sendbuf, 1, 256, file);
					if(rtn > 0){
						send(socket, sendbuf, rtn, 0);
						total += rtn;
					}else {
						break;
						msend("\r\n", socket);
				...
				msend("HTTP/1.1 404 Can't_Found\r\n", socket);
			}
